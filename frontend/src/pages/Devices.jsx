...
export default function Dashboard() {
  ...
  const fetchDevices = async () => {
    const response = await fetch("http://0.0.0.0.1:8000/devices/");
    if (!response.ok) {
              ...
              {isLoading ? (
                ...
                data.map((device) => (
                  <tr key={device.id}>
                    ...
                    <td>{device.name}</td>
                    <td>
                      <a class="btn btn-info" href={`/devicies-single?id=${device.id}`}>
                        <i class="bi bi-pen"></i>  
                      </a>

                      <a class="btn btn-danger" href={`/devicies-single?id=${device.id}`}>
