    ...
    var selectAndSave = function (e, settings, data) {
      ...
      if (data.select.cells !== undefined) {
        for (var i = 0; i < data.select.cells.length; i++) {
          dt.cell(
            data.select.cells[i].row,
            data.select.cells[i].column
          ).select();
        }
      }
 ...
 */
  ...
  function info(api, node) {
    ...
    var add = function (el, name, num) {
      el.append(
        $('<span className="select-item"/>').append(
          api.i18n(
            "select." + name + "s",
            {
              _: "%d " + name + "s selected",
      ...
      if (!$("input", header).length) {
        ...
        var input = $("<input>")
          .attr({
            ...
            type: "checkbox",
            "aria-label":
              dt.i18n("select.aria.headerCheckbox") || "Select all rows",
          })
          .appendTo(header)
          .on("change", function () {
            if (this.checked) {
  ...
  function init(ctx) {
    ...
    api.on("destroy.dtSelect", function () {
      ...
      $(api.rows({ selected: true }).nodes()).removeClass(
    ...
    this.iterator("row", function (ctx, idx) {
      ...
      for (var i = 0; i < dtColumns.length; i++) {
        ...
        if (col.sType === "select-checkbox") {
          ...
          $("input.dt-select-checkbox", dtData.anCells[i]).prop(
            "checked",
            true
          );
  ...
  apiRegisterPlural("cells().select()", "cell().select()", function (select) {
    ...
    this.iterator("table", function (ctx, i) {
      eventTrigger(
        api,
        "select",
        ["cell", api.cells(api[i]).indexes().toArray()],
        true
      );
    });
  ...
  function enabled(dt, config) {
    ...
    if (
      config.limitTo.indexOf("columns") !== -1 &&
    ...
    if (
      config.limitTo.indexOf("cells") !== -1 &&
